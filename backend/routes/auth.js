...
const JWT_SECRET = "Krishnaismakhanchor";
//SIGNUP
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        return res
          .status(410)
          .json({ error: "Try to login with correct credentials", success: false });
      } else {
        const passCompare = await bcrypt.compare(password, user.password);
        if (!passCompare) {
          return res
            .status(410)
            .json({success: false,error: "Try to login with correct credentials"  });
        } else {
